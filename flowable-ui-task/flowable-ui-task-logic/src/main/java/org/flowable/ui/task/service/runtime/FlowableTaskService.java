...
import org.flowable.ui.task.model.runtime.TaskOperationRepresentation;
...
//                //开始
//                ServiceOperation log = new ServiceOperation();
//                if(rep.getDuration()!=null){
//                    log.setServiceID(taskId);
//                    log.setPlanID(rep.getProcessDefinitionId());
//                    log.setInstanceID(rep.getProcessInstanceId());
//                    log.setPlan_start_time(rep.getCreated());
//                    log.setPlan_end_time(rep.getEndDate());
//                    log.setPlan_is_success(1);
//                    log.setResultInfo("成功");
//
//                    serviceOpRepository.save(log);
//                }else{
//                    System.out.println("任务还未全部完成");
//                }
                //结束
    ...
    public TaskOperationRepresentation getTaskOperation(String taskId) {
        User currentUser = SecurityUtils.getCurrentUserObject();
        HistoricTaskInstance task = permissionService.validateReadPermissionOnTask(currentUser, taskId);

        TaskOperationRepresentation rep = null;

        if (StringUtils.isNotEmpty(task.getProcessDefinitionId())) {
            try {
                ProcessDefinition processDefinition = repositoryService.getProcessDefinition(task.getProcessDefinitionId());
                rep = new TaskOperationRepresentation(task, processDefinition);
            ...
            try {
                CaseDefinition caseDefinition = cmmnRepositoryService.getCaseDefinition(task.getScopeDefinitionId());
                rep = new TaskOperationRepresentation(task, caseDefinition);
            ...
            HistoricTaskInstance parentTask = permissionService.validateReadPermissionOnTask(currentUser, task.getParentTaskId());
            rep = new TaskOperationRepresentation(task, parentTask);
        } else {
            rep = new TaskOperationRepresentation(task);
        }
